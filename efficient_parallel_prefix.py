...
"""
* The new algorithm that works efficiently has two distinct parts- the up-sweep phase and the down-sweep phase.

* The up-sweep phase is similar to a single reduce operation to produce the value that is given by the reduce algorithm,
that is  ; in this case we retain the partial sums () that are required the achieve the end result. T
The down-sweep phase will then operate on these partial sums and give us the final result. Let's look at some
pseudocode, starting with the up-sweep phase.
