...
"""
* math api library of device functions.
* Here single and double floating point value are overloaded.
* Intrinsic functions ->  less accurate but faster than math functions
that are built into the NVIDIA CUDA hardware, generally have similar names
to the original function, except that they are preceded with two underscores.
(__sinf)
...
class MonteCarloIntegrator:
    ...
    def definite_integral(self, lo=None, hi=None, samples_per_thread=None, num_blocks=None):
        ...
        self.f(np.int32(samples_per_thread), self.numpy_precision(lo), self.numpy_precision(hi), self.ys, block=block,
               grid=grid)
