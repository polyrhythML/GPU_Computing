/*
* We will apply ctypes to launch a pre-compiled CUDA kernel from Python without
any assistance from PYCUDA.
* We will write a host-side kernel launcher wrapper function in CUDA-C that will
be called directly, which itself has been compiled into a shared binary on linux
or DLL binary on windows.
